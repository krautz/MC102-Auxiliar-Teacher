 ...
 * erro no teste fechado 35 não foi descoberto
 * 2hrs 30 min fazendo o programa, restante procurando erros
 */

int main () {
    int direcao_macho[3001], direcao_femea[3001], tamanho_passos_macho[3001], tamanho_passos_femea[3001], tamanho_macho, tamanho_femea, i, j, flag = 1;
    ...
    int direcao_macho_reverso[3001], tamanho_passos_macho_reverso[3001], reverso = 0, reverte, pos;
    ...
    proporcao = ((float) tamanho_passos_femea[0]) / ((float)tamanho_passos_macho[0]);
    rotacao = direcao_femea[0] - direcao_macho[0];
    
    for (i = 1, flag_1a_fase = 1; i < tamanho_1a_fase && flag_1a_fase; i++) {
        ...
        angulo_m = direcao_macho[i] - direcao_macho[i-1];
        if (angulo_m < 0)
            angulo_m += 8;

        if (!(((float)tamanho_passos_macho[i] * proporcao) == ((float)tamanho_passos_femea[i]) && checa_rotacao == direcao_femea[i] && tamanho_passos_macho[i] > tamanho_passos_femea[i] && angulo_m == angulo_f)) {
            flag_1a_fase = 0;
        }
    }
    ...
    if (flag_1a_fase == 0) {
        ...
        if (!(tamanho_2a_fase_f >= tamanho_2a_fase_m)) {
            ...
            if (tamanho_macho - tamanho_2a_fase_m == tamanho_femea - tamanho_2a_fase_f) {
                proporcao = ((float) tamanho_passos_femea[tamanho_2a_fase_f]) / ((float)tamanho_passos_macho[tamanho_2a_fase_m]);
                for (i = tamanho_2a_fase_f + 1, j = tamanho_2a_fase_m + 1, flag_3a_fase = 1; flag_3a_fase && i < tamanho_femea && j < tamanho_macho; i++, j++) {
                    if (((float) tamanho_passos_macho[j] * proporcao) == ((float) tamanho_passos_femea[i])) {
                        ...
                        if (!(angulo_m == angulo_f || angulo_m + 1 == angulo_f || angulo_m - 1 == angulo_f || (angulo_m + 1 == 8 && angulo_f == 0) || (angulo_m == 0 && angulo_f == 7))) {
                            flag_3a_fase = 0;
                        }
                    } else {
                        flag_3a_fase = 0;
    ...
    if (reverso) {
        ...
        /* determinando os passos da 1a danca */
        for (tamanho_1a_fase = 0, flag = 1; flag; tamanho_1a_fase++) {
            dir[direcao_femea[tamanho_1a_fase]] += tamanho_passos_femea[tamanho_1a_fase];
            if (dir[0] == dir[4] && dir[1] == dir[5] && dir[2] == dir[6] && dir[3] == dir[7]) {
                flag = 0;
            }
        ...
        if (flag_1a_fase == 0) {
        	printf("Incompatibilidade total.\n");
                ...
                if (!(tamanho_2a_fase_f >= tamanho_2a_fase_m)) {
                    printf("Incompatibilidade total.\n");
                } else if (tamanho_macho - tamanho_2a_fase_m == tamanho_femea - tamanho_2a_fase_f) {
